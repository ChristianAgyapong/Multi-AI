/**
 * flashcards.js
 * Handles fetching, generating, and rendering flashcards in the UI.
 */
...
async function generateFlashcards() {
    // Get text from recent chat messages
    ...
    // Grab the text from the last few messages
    let recentText = "";
    const messages = Array.from(chatContainer.children).slice(-6); // last 3 pairs
    messages.forEach(msg => {
        recentText += msg.innerText + "\n\n";
    });
...
function renderFlashcards() {
    ...
    if (flashcardsData.length === 0) {
        container.innerHTML = '<p style="color: var(--text-muted);">No flashcards yet. Generate some from your chat!</p>';
        return;
    }
        ...
        cardEl.innerHTML = `
            <div class="flashcard-inner">
                <div class="flashcard-front">
                    <h3>${card.front}</h3>
                    <p style="font-size: 0.8rem; color: var(--text-muted); margin-top: auto; padding-top: 1rem;">Click to flip</p>
                </div>
